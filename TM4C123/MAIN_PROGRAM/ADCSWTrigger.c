...
//  SYSCTL_RCGC0_R |= 0x00010000; // 1) activate ADC0 (legacy code)
  ...
  ADC1_SSMUX2_R = 0x0089;         // 12) set channels for 
	//0x0089
...
void ADC_In89(uint32_t data[2]){ 
  ...
  data[0] = ADC1_SSFIFO2_R&0xFFF;  // 3A) read first result
  data[1] = ADC1_SSFIFO2_R&0xFFF;  // 3B) read second result
